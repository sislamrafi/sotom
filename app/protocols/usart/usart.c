...
void configUSART(USART_t* usart, USART_CONFIG config, uint32_t baudrate) {
  ...
  // config pins
  pinConfig(config.RX.GPIO, config.RX.pin, GPIO_ALTERNET | config.RX.af | GPIO_HIGH_SPEED | GPIO_LOCK_PIN);
  pinConfig(config.TX.GPIO, config.TX.pin, GPIO_ALTERNET | config.TX.af | GPIO_HIGH_SPEED | GPIO_LOCK_PIN);
