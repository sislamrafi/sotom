...
def get_symbol_details(name,_target=None,bit=32):
    ...
    global sub_section
    global filename
    output = {'name':None,'type':None,'address':None, 'value':None, 'section':None, 'sub_section':None, 'file':None}
    try:
        ...
        if output['type'] == 'address':
            ...
            if _target != None:
                value = None
                if bit == 32:
                    value = _targer.read32(output['address'])
                elif bit == 16:
                    value = _targer.read16(output['address'])
                else:
                    value = _targer.read8(output['address'])
                output[value] = value
